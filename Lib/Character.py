...
class Character(object):
    ...
    def apply_class(self, class_obj):
        ...
        **Skills** (a dictionary mapping skills to their starting value), **Choices** (a dictionary
        with keys **Skills**, **Number** and sometimes **Subskills**, which is used when a class
        is allowed to choose one from a number of skills), **Subskills** (an array of dictionaries;
        each inner one contains the keys **Skill**, **Sub**, and **Value** - which are used to set
        up skills the character class gets when sub-skills like Craft (Microelectronics) are
        specified in the class description, or when a skill with sub-skills is included multiple
        times in the class), and **Bonds** (the number of bonds the character class gets)
        :return: None
        :rtype: None
        """
